...
+ [Why synchronization is bad for performance even in single-thread application](#why-synchronization-is-bad-for-performance-even-in-single-thread-application)
...
## Why synchronization is bad for performance even in single-thread application
https://stackoverflow.com/a/1671097
https://stackoverflow.com/a/8524882
...
Ключевые слова `transient` и `native` к многопоточности никакого отношения не имеют, первое используется для указания полей класса, 
которые не нужно сериализовать, а второе - сигнализирует о том, что метод реализован в платформо-зависимом коде.
...
например `getAndIncrement()` – атомарная замена оператора инкремента, его можно использовать, 
